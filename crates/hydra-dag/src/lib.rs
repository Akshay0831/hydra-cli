    ...
    T: Send + Sync + 'static,
{
    ...
    pub fn topological_order(&self) -> Option<Vec<String>> {
        ...
        for task_id in self.task_ids() {
            if !remaining_deps.contains_key(task_id)
                && !self.tasks[task_id].dependencies().is_empty()
            {
        ...
        for task_id in self.task_ids() {
            let has_dependencies = self.tasks[task_id]
                .dependencies()
                .iter()
                .any(|dep| self.tasks.contains_key(dep));
    ...
    pub fn has_cycles(&self) -> bool {
        self.validate_dependencies().is_ok() && self.topological_order().is_none()
    }
    ...
    pub async fn execute_sequential(&self) -> ExecutionResult<T> {
        ...
        if let Err(error) = self.validate_dependencies() {
            return ExecutionResult {
                successful: results,
                failed: vec![("graph_invalid".to_string(), error.to_string())],
    ...
    pub async fn execute_concurrent(
        ...
        if max_concurrency == 0 {
            return ExecutionResult {
                successful: HashMap::new(),
                failed: vec![(
                    "graph_invalid".to_string(),
                    "Maximum concurrency must be greater than zero".to_string(),
                )],
                strategy,
            };
        }
...
mod tests {
    ...
    async fn test_sequential_tasks() {
        ...
        let task1 = SimpleTask::new("task1".to_string(), vec![], || Ok("result1".to_string()));

        let task2 = SimpleTask::new("task2".to_string(), vec!["task1".to_string()], || {
    ...
    async fn test_parallel_tasks() {
        ...
        let task2 = SimpleTask::new("task2".to_string(), vec![], || Ok("result2".to_string()));
    ...
    async fn test_diamond_dependencies() {
        ...
        let task3 = SimpleTask::new("task3".to_string(), vec!["task1".to_string()], || {
    ...
    async fn test_cycle_detection() {
        ...
        assert!(
            result1.is_ok(),
            "Adding task1 should succeed when task2 doesn't exist yet"
        );
        ...
        assert!(
            result2.is_err(),
            "Adding task2 should fail because it creates a cycle"
        );
    }
    ...
    async fn test_topological_order() {
        ...
        graph
            .add_tasks(vec![Box::new(task1), Box::new(task2), Box::new(task3)])
            .unwrap();
    ...
    async fn test_cycle_detection_topological() {
        ...
        let task1 = SimpleTask::new("task1".to_string(), vec!["task2".to_string()], || {
            Ok("result1".to_string())
        });
        ...
        assert!(
            !graph.has_cycles(),
            "Graph should not have cycles after failed task addition"
        );

        // Verify the graph still has task1 and works normally
        assert_eq!(graph.tasks.len(), 1, "Graph should contain only task1");
        assert!(
            graph.tasks.contains_key("task1"),
            "Graph should contain task1"
        );
    }
    ...
    async fn test_task_with_dependencies() {
        ...
        let task1 = SimpleTask::new("task1".to_string(), vec![], move || {
            execution_order_clone1.fetch_add(1, Ordering::SeqCst);
            Ok("result1".to_string())
        });

        let task2 = SimpleTask::new("task2".to_string(), vec!["task1".to_string()], move || {
    ...
    async fn test_concurrent_execution() {
        ...
        let result = graph
            .execute_concurrent(2, ExecutionStrategy::CollectFailures)
            .await;
    ...
    #[tokio::test]
    async fn missing_dependency_is_rejected_before_execution() {
        let mut graph = TaskGraph::new();
        graph
        ...
        let result = graph
            .execute_concurrent(1, ExecutionStrategy::CollectFailures)
            .await;
        assert_eq!(result.successful_count(), 0);
        assert_eq!(
            result.get_error("graph_invalid").map(String::as_str),
            Some("Task task depends on missing task missing")
        );
    }
    ...
    async fn zero_concurrency_is_rejected() {
        ...
        let result = graph
            .execute_concurrent(0, ExecutionStrategy::CollectFailures)
            .await;
        assert_eq!(
            result.get_error("graph_invalid").map(String::as_str),
